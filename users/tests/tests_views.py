...
class TestUserViews(TestCase):
    ...
    def test_unfollow_a_user_if_logged_in(self):
        ...
        self.assertEquals(response.status_code, 200)
        self.assertEquals(json_status['status'], 'success')
        # Checking if entry deleted from  DB
        f = Followers.objects.filter(id=1).exists()
        self.assertFalse(f)

    def test_view_followers_if_not_logged_in(self):
        response = self.client.get(
            reverse('view_followers', args=[self.user1.username]))
        self.assertEquals(response.status_code, 302)
        self.assertRedirects(
            response, f'/login/?next=/view_followers/{self.user1.username}/')

    def test_view_followers_if_logged_in(self):
        login = self.client.login(
            username=self.user1.username, password='testing@123')
        self.assertTrue(login)
        response = self.client.get(
        ...
        self.assertEquals(response.status_code, 200)
        self.assertTemplateUsed(response, 'users/followers.html')
        #print(response.context['no_of_following'], response.context['followers'])

    def test_update_profile_if_not_logged_in_GET(self):
        response = self.client.get(reverse('update_profile'))
        self.assertEquals(response.status_code, 302)
        self.assertRedirects(response, '/login/?next=/update-profile/')

    def test_update_profile_if_not_logged_in_POST(self):
        response = self.client.post(reverse('update_profile'))
        self.assertEquals(response.status_code, 302)
        self.assertRedirects(response, '/login/?next=/update-profile/')

    def test_update_profile_if_logged_in_GET(self):
        login = self.client.login(
            username=self.user1.username, password='testing@123')
        self.assertTrue(login)
        response = self.client.get(reverse('update_profile'))
    ...
    def test_update_profile_if_logged_in_POST(self):
        login = self.client.login(
            username=self.user1.username, password='testing@123')
        self.assertTrue(login)
        new_email = 'update_test_mail1@gmail.com'
        response = self.client.post(reverse('update_profile'), data={
                                    'email': new_email})
        self.assertEquals(response.status_code, 302)
        p = User.objects.get(id=self.user1.id)
        self.assertRedirects(response, reverse('profile', args=[self.user1.username]))
        self.assertEquals(p.email, new_email)
